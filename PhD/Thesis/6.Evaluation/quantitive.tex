...
The results are now investigated, starting by discussing the way in which the results support the comparison hypothesis. Subsequently, results that contradict the hypothesis are analysed in more detail. The analysis led to the discovery of two limitations of the conservative copy implementation in Flock, which are also discussed below.
...
Figure~\ref{fig:subtyping} shows part of the GMF Graph metamodel prior to evolution. In the real metamodel, the \texttt{figure} and \texttt{accessor} features are references to other metamodel classes, rather than attributes. When the metamodel evolved, the types of the \texttt{figure} and \texttt{accessor} features were changed. Here, let us assume that their types were changed from a string to an integer. The actual metamodel changes are described in Section~\ref{subsec:gmf_graph}.
