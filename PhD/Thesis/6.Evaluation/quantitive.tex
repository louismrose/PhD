...
% Evaluating conservative copy
% Productivity: More concise migration strategies? Less to write, but could the other code be generated?
% Understandability: More concise migration strategies? Less to read, but could ignore the repetitive sections. Proposing a dedicated language for migration, which allows patterns to be recognised (understood) and re-used (reduce effort, increase productivity)
% But could I argue that by generating code or ignoring the repetitive sections one is doing a conservative copy, but implementing it with a different style of language?

%% Counting model operations is an approximation of conciseness. I could have counted LOC, but that would be biased by differences in the languages

% Other approaches
%% Metamodel specific terminology: compare number of language-specific and number of metamodel-specific terms. 
%% McCabe or Halstead complexity. Measures of programs, rather than programming languages. Can write the same program to have higher or lower McCabe / Halstead scores in the same programming language. 
