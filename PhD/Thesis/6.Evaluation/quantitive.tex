...
Figure~\ref{fig:subtyping} shows part of the GMF Graph metamodel prior to evolution. In the real metamodel, the \texttt{figure} and \texttt{accessor} features are references to other metamodel classes, rather than attributes. When the metamodel evolved, the types of the \texttt{figure} and \texttt{accessor} features were changed. Here, let us assume that their types were changed from a string to an integer. The actual metamodel changes are described in Appendix~\ref{MigrationStrategies}.
