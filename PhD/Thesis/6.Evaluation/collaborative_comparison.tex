...
\label{subsec:collaborative_comparison}
...
In Figure~\ref{fig:petri_nets_original_mm_repeated}, a Petri \mm{Net} comprises \mm{Place}s and \mm{Transition}s. A \mm{Place} has any number of \mm{src} or \mm{dst} \mm{Transition}s. Similarly, a \mm{Transition} has at least one \mm{src} and \mm{dst} \mm{Place}. In this example, the metamodel in Figure~\ref{fig:petri_nets_original_mm_repeated} is to be evolved to support weighted connections between \mm{Place}s and \mm{Transition}s and between \mm{Transition}s and \mm{Place}s.
...
\paragraph{Changing the migration strategy}
...
% LMR  I've commented this out because I think, in its current form, this might be too technical for this section. TODO Perhaps it could be rephrased?
% The separation of original and evolved metamodel made it also difficult to decide where to better specify the migration. It was thus necessary to switch between the two metamodels back and forth to know where to best implement the migration strategy. When doing this, problems are easily introduced, since the consistent usage of original and evolved metamodel is not verified.
...
% LMR trimmed for space
%implemented in the Epsilon Object Language (EOL) -- the base language of the Epsilon family.
% However, about 60 lines of code are superfluous by calling this subroutine from the rules required for every concrete subclass.
...
\subparagraph{AML.} The AML code generator takes a conservative approach to naming variables, to minimise the chances of duplicate variable names. Hence, some of the generated code can be difficult to read and hard to re-use if the generated transformation has to be completed by hand. When a complete transformation can be generated by AML, clarity is not as important.
...
% - however this can vary according to the adopted programming style.
