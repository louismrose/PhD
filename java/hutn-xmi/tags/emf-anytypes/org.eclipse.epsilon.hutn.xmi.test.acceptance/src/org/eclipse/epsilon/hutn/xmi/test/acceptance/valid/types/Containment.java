...
public class Containment extends ValidAcceptanceTest {
	...
	public static void setup() throws URISyntaxException {
		validAcceptanceTest(FileUtil.getFile("TypesReferences.model", Containment.class));
		
		pkg   = spec.getObjects().get(0);
		model = pkg.getClassObjects().get(0);
		slot = model.findSlot("contents");
	}
	...
	public void slotHasCorrectContent() {
		assertEquals("Pet",    ((ContainmentSlot)slot).getClassObjects().get(0).getType());
		assertEquals("Family", ((ContainmentSlot)slot).getClassObjects().get(1).getType());
	}
}
